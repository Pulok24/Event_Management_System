...
public class AuthController {
    ...
    @GetMapping("/find")
    public ResponseEntity<List<EventDto>> getByNameCityDate(@RequestParam String name, @RequestParam String city){
    	List<Event> events=eventRepository.findEvent(name, city);
    	if(events.isEmpty()) {
    		throw new AppException("No Event Found", HttpStatus.NOT_FOUND);
    	}
    	List<EventDto> eventDtoList = eventMapper.listEventToDto(events);
    	return ResponseEntity.ok(eventDtoList);
//    	return ResponseEntity.status(HttpStatus.OK).body(eventMapper.listEventToDto(events));
//    	return ResponseEntity.status(HttpStatus.OK).body(eventDtoList);
    ...
    public ResponseEntity<Attendee> addAttendee(@RequestBody @Valid Attendee attendee){
    	Event event= eventRepository.getById(attendee.getEventid());
    	attendee.setQuantity(attendee.getQuantity()*event.getPrice());
